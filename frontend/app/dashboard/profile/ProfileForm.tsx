...
export default function ProfileForm({ user }: { user: UserProp | null }) {
    const [toast, setToast] = useState<{ msg: string; type: "success" | "error" } | null>(null);
    const [loading, setLoading] = useState(false);

    const nameParts = (user?.name || "").split(" ");
    const initialFirstName = nameParts[0] || "Local Dev";
    const initialLastName = nameParts.slice(1).join(" ") || "User";
    const initialEmail = user?.email || "dev@localhost";

    // Core Profile State initialized from user session
                    ...
                    <div className="bg-card border border-border p-6 rounded-3xl shadow-sm text-center">
                        <div className="relative w-32 h-32 mx-auto mb-4 group cursor-pointer">
                            {user?.picture ? (
                                ...
                                <img src={user.picture} alt="Avatar" className="w-full h-full rounded-3xl object-cover shadow-lg" />
                            ) : (
